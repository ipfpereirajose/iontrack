  ...
  return (
    <button
      ...
      style={{
        ...
        padding: '0.875rem 2.5rem', background: 'var(--primary)',
        color: 'white', border: 'none', borderRadius: '12px',
        fontWeight: 700, fontSize: '1rem', cursor: pending ? 'not-allowed' : 'pointer',
        opacity: pending ? 0.7 : 1,
      }}
    >
      {pending ? <Loader2 size={20} className="animate-spin" /> : <UserPlus size={20} />}
...
export default function NewPortalWorkerPage() {
  ...
  const inputStyle: React.CSSProperties = {
    ...
    padding: '0.875rem 1rem', color: 'var(--text-main)',
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      <header style={{ marginBottom: '2.5rem' }}>
        ...
        <h1 style={{ fontSize: '2.5rem', fontWeight: 900, marginBottom: '0.5rem' }}>Registrar Nuevo TOE</h1>
        <p style={{ color: 'var(--text-muted)' }}>Ingresa los datos del trabajador ocupacionalmente expuesto.</p>
      </header>
      ...
      <form action={clientAction} className="glass-panel" style={{ display: 'flex', flexDirection: 'column', gap: '2rem', padding: '3rem', borderRadius: '24px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '1.5rem' }}>
          ...
          <div>
            ...
            <input required name="birth_year" type="number" min={1940} max={2005} placeholder="Ej: 1985" style={inputStyle} />
          </div>
        </div>
