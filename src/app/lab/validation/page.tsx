...
export default async function ValidationPage() {
  ...
  return (
    <div>
      ...
      {error && (
        <div className="clean-panel" style={{ border: '1px solid var(--state-danger)', color: 'var(--state-danger)', marginBottom: '2rem', background: 'rgba(239, 68, 68, 0.05)' }}>
      ...
      {!doses || doses.length === 0 ? (
        <div className="clean-panel" style={{ textAlign: 'center', padding: '6rem' }}>
          <CheckCircle size={64} color="var(--state-safe)" style={{ marginBottom: '1.5rem', opacity: 0.3 }} />
            ...
            <tbody>
              {doses.map((dose: any) => (
                <tr key={dose.id}>
                  ...
                  <td>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', fontSize: '0.875rem', color: 'var(--primary-teal)', fontWeight: 600 }}>
                      <Building2 size={16} />
                      {dose.toe_workers?.companies?.name}
                    </div>
                  </td>
                  <td>
                    <span style={{ background: '#f1f5f9', padding: '0.4rem 0.8rem', borderRadius: '8px', fontSize: '0.875rem', fontWeight: 700 }}>
                  ...
                  <td>
                    <div style={{ display: 'flex', alignItems: 'baseline', gap: '0.25rem' }}>
                      <span style={{ 
                        fontSize: '1.35rem', 
                        fontWeight: 900, 
                        color: dose.hp10 >= 1.66 ? 'var(--state-danger)' : dose.hp10 >= 1.328 ? 'var(--state-warning)' : 'var(--text-main)' 
                      }}>
                        {dose.hp10}
                      </span>
