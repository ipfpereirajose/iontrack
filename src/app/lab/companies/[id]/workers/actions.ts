...
export async function createWorker(companyId: string, formData: FormData) {
  ...
  if (!profile || !company || profile.tenant_id !== company.tenant_id) {
    throw new Error(
      "No tienes permiso para agregar trabajadores a esta empresa",
    );
  }
  ...
  const { error } = await supabase.from("toe_workers").insert([
    {
      first_name,
      last_name,
      ci,
  ...
  if (error) {
    ...
    if (
      error.message.includes("worker_code") &&
      error.message.includes("unique")
    ) {
      throw new Error(
        "El código de trabajador ya existe para esta empresa. Use uno diferente.",
      );
    }
