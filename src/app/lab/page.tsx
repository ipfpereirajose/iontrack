...
export default async function LabHomePage({
  ...
  // 0. Fetch company IDs for this tenant
  ...
  // 1. Fetch Stats & Remaining Data (MINIMAL)
  const [
    ...
    adminSupabase
      .from("doses")
      .select("id, month, year, hp10, status, toe_workers!inner(first_name, last_name, ci, companies!inner(name))")
      .eq("status", "pending")
  ...
  return (
    <div style={{ padding: "1.5rem", maxWidth: "1600px", margin: "0 auto" }}>
      ...
      >
        ...
        <div className="clean-panel" style={{ borderLeft: `4px solid ${criticalDoses?.length ? "var(--state-danger)" : "var(--state-safe)"}` }}>
          <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "1rem" }}>
            <span style={{ fontSize: "0.75rem", fontWeight: 800, color: "var(--text-muted)", textTransform: "uppercase" }}>Alertas Críticas</span>
            <AlertTriangle size={18} color={criticalDoses?.length ? "var(--state-danger)" : "var(--text-muted)"} />
          </div>
          <div style={{ fontSize: "2.25rem", fontWeight: 900, color: criticalDoses?.length ? "var(--state-danger)" : "inherit" }}>{criticalDoses?.length || 0}</div>
          <p style={{ fontSize: "0.75rem", color: criticalDoses?.length ? "var(--state-danger)" : "var(--state-safe)", marginTop: "0.5rem", fontWeight: 700 }}>
            {criticalDoses?.length ? "🚨 Sobre-exposición" : "✓ Sin incidencias"}
          </p>
        </div>
      </div>
