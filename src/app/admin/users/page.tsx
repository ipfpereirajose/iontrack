...
export default async function AdminManagement() {
  ...
  if (pError || aError) {
    return <div className="p-8 text-red-500">Error al cargar administradores.</div>;
  }
  ...
  return (
    <div className="space-y-8">
          ...
          <tbody>
            {users.map((user) => (
              <tr key={user.id}>
                ...
                <td>
                  <span className={`badge ${user.role === 'superadmin' ? 'badge-danger' : 'badge-success'}`}>
                ...
                <td style={{ textAlign: 'right' }}>
                  <div style={{ display: 'flex', gap: '0.5rem', justifyContent: 'flex-end' }}>
                    ...
                    <form action={async () => {
                      ...
                      <button className="btn btn-secondary" style={{ padding: '0.5rem', color: '#ef4444' }} title="Eliminar Acceso">
                        <Trash2 size={18} />
                      </button>
                    </form>
                  </div>
      ...
      <div style={{ 
        ...
        <div>
          <h3 style={{ fontWeight: 800, marginBottom: '0.25rem' }}>Advertencia de Seguridad</h3>
          <p style={{ fontSize: '0.9rem', color: 'var(--text-muted)' }}>
            Los cambios en los roles de administrador tienen efecto inmediato sobre la capacidad de modificar infraestructura y datos críticos. 
            Todas las acciones en este panel son registradas en el log de auditoría inmutable.
          </p>
        </div>
      </div>
