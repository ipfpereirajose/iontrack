...
export default function InteractiveTrendChart({ workerIds, targetYear }: InteractiveTrendChartProps) {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const processedData = months.map((name, index) => {
        ...
        const activeWorkers = new Set(monthDoses.map(d => d.toe_worker_id)).size;
    ...
    <div className="clean-panel" style={{ minHeight: "450px", position: "relative" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "1rem" }}>
        <div style={{ display: "flex", alignItems: "center", gap: "0.75rem" }}>
          ...
          <h2 style={{ fontSize: "1.25rem", fontWeight: 800 }}>Tendencia de Carga Dosimétrica (Promedio)</h2>
        </div>
        <button 
          onClick={loadData} 
          disabled={loading}
      ...
      <div style={{ marginTop: "1rem", fontSize: "0.75rem", color: "var(--text-muted)", display: "flex", justifyContent: "space-between" }}>
        <span>* Valores expresados en mSv promedio por trabajador activo.</span>
