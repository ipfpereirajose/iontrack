...
export default function RegulatoryReportButton({ month, year, tenantName }: Props) {
  const downloadReport = async () => {
    // 1. Fetch all approved doses for this lab and period
    // In a real app, this would be a server action
    const response = await fetch(`/api/reports/regulatory?month=${month}&year=${year}`);
  ...
  return (
    <button onClick={downloadReport} className="btn btn-secondary" style={{ gap: '0.75rem' }}>
      <Download size={18} />
