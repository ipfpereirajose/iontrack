import { Users, Calendar, ShieldCheck, CheckCircle2, Clock } from "lucide-react";
import { createClient } from "@/utils/supabase/server";

export default async function PortalDashboardWidget({ companyId }: { companyId: string }) {
  const supabase = await createClient();

  // Fetch Stats in parallel
  const [
    { count: toeCount, error: toeError },
    { data: latestDoses, error: dosesError }
  ] = await Promise.all([
    supabase.from("toe_workers").select("*", { count: "exact", head: true }).eq("company_id", companyId).eq("status", "active"),
    supabase.from("doses").select("*, toe_workers!inner(*)").eq("toe_workers.company_id", companyId).order("year", { ascending: false }).order("month", { ascending: false }).limit(5)
  ]);

  if (toeError || dosesError) {
    console.error("Error loading PortalDashboardWidget", toeError || dosesError);
  }
